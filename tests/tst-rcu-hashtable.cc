...
struct test_element {
    test_element(unsigned val) : _val(val), _state(state::initialized) {
        ctors.fetch_add(std::memory_order_relaxed);
    }
    ~test_element() {
        dtors.fetch_add(std::memory_order_relaxed);
    ...
    void validate() { assert(_state == state::initialized); }
...
BOOST_AUTO_TEST_CASE(test_rcu_hashtable_basic) {
    {
        osv::detail::rcu_hashtable<test_element> ht;
        ht.by_owner().emplace_front(5);
        bool r1, r2, r3;
        WITH_LOCK(osv::rcu_read_lock) {
            auto rht = ht.for_read();
            auto i1 = rht.find(test_element(5));
            r1 = i1 != rht.end();
            r2 = i1->_val == 5;
            auto i2  = rht.find(test_element(6));
            r3 = i2 == rht.end();
        }
        BOOST_REQUIRE(r1);
        BOOST_REQUIRE(r2);
...
void do_reads(osv::detail::rcu_hashtable<test_element>& ht,
        size_t range,
...
BOOST_AUTO_TEST_CASE(test_rcu_hashtable) {
    ...
    static const size_t iterations = CONF_debug_memory ? 1000000 : 1000000000;
    ...
    {
        {
            ...
            for (size_t i = 0; i < iterations; ++i) {
                ...
                case 0: { // insert
                    ...
                    mht.emplace_front(value);
                    ++s.insertions_lower_bound;
                    ++size;
                    break;
                }
                case 1: {// delete
                    auto lb = s.insertions_lower_bound - s.removals_upper_bound;
                    auto hash = [](size_t value) -> size_t { return value; };
                    auto compare = [](unsigned x, const test_element& y) { return x == y._val; };
                    auto i = mht.find(value, hash, compare);
                    if (lb == 0) {
                        assert(i == mht.end());
                        BOOST_REQUIRE(i == mht.end());
                    } else {
