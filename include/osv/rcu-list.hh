...
{
    std::unique_ptr<element> e{new element(_first.read_by_owner(), data)};
    _first.assign(e.release());
}
...
{
    std::unique_ptr<element> e{new element(_first.read_by_owner(), std::forward<Args>(args)...)};
    _first.assign(e.release());
}
